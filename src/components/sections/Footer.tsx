...
export function Footer() {
  ...
  return (
    <footer style={{ background: 'var(--bg-alt)', borderTop: '1px solid var(--border)', padding: '20px 24px' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <span className="f-display" style={{ fontSize: 15, fontWeight: 700, color: 'var(--accent)' }}>
          Sugumaran S
        </span>
        <div style={{ display: 'flex', gap: 18 }}>
          {links.map(({ icon: Icon, href, label }) => (
            <a key={label} href={href} target={href.startsWith('http') ? '_blank' : undefined} rel="noopener noreferrer" aria-label={label}
              style={{ color: 'var(--txt-3)', transition: 'color 0.2s' }}
        ...
        <span className="f-mono" style={{ fontSize: 11, color: 'var(--txt-3)' }}>© {new Date().getFullYear()}</span>
      </div>
    </footer>
  )
}
